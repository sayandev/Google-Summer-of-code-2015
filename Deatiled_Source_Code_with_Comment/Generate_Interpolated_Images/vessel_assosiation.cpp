...
void get_boundary_centroid(cv::Mat dialated,vector<vector<Point>> &blobs_sorted, vector<double> &bolb_area_sorted,vector<Point> &bolb_center1_sorted, vector<Vec4i> &hierarchy)
{
			...
			for (int i = 0; i < bolb_area.size(); i++ ) {
				bolb_center1_sorted.push_back(bolb_center1.at(vp[i].second));

				
				}
			
			for (int i = 0; i < top; i++ ) {
				blobs_sorted.push_back(blobs.at(vp[i].second));
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	for(; in2 >= 0; in2--) 
		{
			...
			for (int j = 0; j < current_frame_parent.size(); j++ ) 
			{
				...
				if (parent_id!=-2147483648)
				{
					vector<int> vessel(bounds_local.size(), 0);
					vessel[in2+1] = j+1;
					...
					if ((k==1)&&(parent_id!=-2147483648))
					{
						vector<vector<Point>> temp_bound;
						temp_bound=bounds_local[k-1];
						vessel_bound[k-1]=temp_bound[parent_id];
						vessel[k-1] = parent_id+1;
					}
					vessels_ids.push_back(vessel);
	...
	//get first column
	cv::Mat col_one= vessel_mat.col(0);
	...
	for(int y = 0; y < vessel_mat.rows; y++){
	   vessel_mat_sorted.at(idx.at(y))=vessels_ids.at(y);	 
	}
	// Destroy Mat objects
	vessel_mat.release();
	col_one.release();
	...
	for(int i=0; i<vessel_mat_sorted.size(); ++i)
	{
		...
		{
			vector<int> a=vessel_mat_sorted.at(i);
			int j=i+1;
			while (j<vessel_mat_sorted.size())
			{
				vector<int> b=vessel_mat_sorted.at(j); 
				vector<int> aa;
				vector<int> bb;
			...
			if(j!=vessel_mat_sorted.size()+100)//a is a good vessel
			{
				...
				selected_vessel_mat.push_back(a);
			}
	...
	//get first column
	vector<int> vessel_mat_col=selected_vessel_mat_t[0] ;
		...
		if (C_vessel_mat_col[i]!=0)
		{
			...
			if(smax==emin)
			{
				...
				for(int j=0;j<one_vessel.size();j++)
				{
					...
					drawContours(BW1,trytraj1,0,Scalar(255, 255, 255),CV_FILLED,8);
					imshow("image5", BW1);	
					waitKey(5);
